...
- (void)setDefaultSettings {
    ...
    self.panMode = MFSideMenuPanModeCenterViewController| MFSideMenuPanModeSideMenu;
    self.viewHasAppeared = NO;
    self.darkenStatusBarWhenMenuOpens = YES;
}
...
//        [[self.rightMenuViewController view] removeGestureRecognizer:[self centerTapGestureRecognizer]];
    }
}
...
//        [[self.rightMenuViewController view] addGestureRecognizer:[self centerTapGestureRecognizer]];
    }
}
...
- (void) setRightSideMenuFrameToClosedPosition {
    ...
    // set height of rightview follow self.view
...
- (void)orientationDidChange:(NSNotification *)noti{
    ...
    CGRect statusBarFrame = self.statusBarBackgoundView.frame;
    CGRect rightMenuViewFrame = self.rightMenuViewController.view.frame;
    UIInterfaceOrientation orientation = [UIApplication sharedApplication].statusBarOrientation;
    if ((orientation == UIInterfaceOrientationLandscapeLeft) || (orientation == UIInterfaceOrientationLandscapeRight)) {
        if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
             rightMenuViewFrame.size.height = self.view.bounds.size.height;
        } else {
            rightMenuViewFrame.size.height = self.view.bounds.size.height;
        }
        statusBarFrame.size.width = self.view.bounds.size.width;
        ...
        statusBarFrame.size.width = self.view.bounds.size.height;
        if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
            rightMenuViewFrame.size.height = self.view.bounds.size.height;
        } else {
            rightMenuViewFrame.size.height = self.view.bounds.size.height;
    ...
    self.statusBarBackgoundView.frame = statusBarFrame;
    self.rightMenuViewController.view.frame = rightMenuViewFrame;
}
@end